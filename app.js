...
function ensureUniqueCategoryId(label) {
  const base = makeCategoryId(label);
  let id = base;
  let i = 2;
  while (catalogs.categories.some(c => c.id === id)) {
    id = `${base}_${i++}`;
  }
  return id;
}
...
function resolveCategoryId(value) {
  ...
  const fallback = DEFAULT_CATEGORIES.find(c => c.id === raw);
  const cat = {
    id: raw,
    label: fallback?.label || raw,
    color: fallback?.color || DEFAULT_CATEGORIES[catalogs.categories.length % DEFAULT_CATEGORIES.length].color
  };
...
function renderMckOptions(groupId, values, checkedValues=[]) {
  ...
  el.innerHTML = values.map(v => {
    ...
    return `<label class="mck-label${isChecked ? ' checked' : ''}" onclick="toggleMck(this)">
      <input type="checkbox" value="${escapeAttr(v)}"${isChecked ? ' checked' : ''}>${escapeHtml(v)}
    </label>`;
  }).join('');
}
...
function renderTaskCatalogInputs() {
  ...
  if (cat) {
    ...
    cat.innerHTML = catalogs.categories.map(c =>
      `<option value="${escapeAttr(c.id)}">${escapeHtml(c.label)}</option>`
    ).join('');
...
function addCategorySetting() {
  ...
  catalogs.categories.push({
    id: ensureUniqueCategoryId(label),
    label,
    color: colorInput?.value || '#378ADD'
  });
  labelInput.value = '';
  persistCatalogState();
}
...
function updateCategorySetting(id, field, value) {
  ...
  if (field === 'color' && /^#[0-9a-f]{6}$/i.test(String(value || ''))) {
    cat.color = value;
  }
  persistCatalogState();
}
...
function renameListSetting(type, idx, nextValue) {
  ...
  if (type === 'owners') {
    tasks.forEach(t => { t.owners = uniqueStrings((t.owners || []).map(o => o === oldValue ? value : o)); });
  } else {
    tasks.forEach(t => { t.tags = uniqueStrings((t.tags || []).map(tag => tag === oldValue ? value : tag)); });
  }
  persistCatalogState();
}
...
function deleteListSetting(type, idx) {
  ...
  if (type === 'owners') {
    tasks.forEach(t => { t.owners = (t.owners || []).filter(o => o !== oldValue); });
  } else {
    tasks.forEach(t => { t.tags = (t.tags || []).filter(tag => tag !== oldValue); });
  }
  persistCatalogState();
}
...
function load() {
  try {
    ...
    else tasks = tasks.map(normalizeTaskForUi);
    ensureCatalogsCoverTasks(tasks);
